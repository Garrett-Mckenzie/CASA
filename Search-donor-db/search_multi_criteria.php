...
if (empty($query_types) || in_array('all_donors', $query_types)) {
    ...
    if (in_array('donor_name', $query_types) && isset($_GET['donor_name_query'])) {
        ...
        if (strlen($event_query) > 0) {
            ...
            // Add the necessary JOINs to link donors to events
            // Assuming two tables: 'donations' (links donors to events) and 'dbevents' (event details)
            ...
            // Check if the query is a numeric ID or a string name
            if (is_numeric($event_query)) {
                // Match by Event ID
                $where_clauses[] = "(e.id = '$safe_event_query')";
            } else {
                // Match by Event Name (using LIKE for partial matches)
                $where_clauses[] = "(e.event_name LIKE '%$safe_event_query%')";
            }
        }
    }
